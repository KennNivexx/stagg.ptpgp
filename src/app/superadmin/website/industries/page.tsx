...
const defaultIndustry: Industry = { name: "", icon: "", description: "" };
...
export default function IndustriesCMS() {
  ...
  useEffect(() => {
    getWebsiteSettings()
      .then((settings) => {
        const s = settings.industries || {};
        setForm({
          show: (s.show as boolean) ?? true,
          title: (s.title as string) || "Industri yang Kami Layani",
          subtitle: (s.subtitle as string) || "",
          industries:
            (s.industries as Industry[])?.length
              ? (s.industries as Industry[])
  ...
  const handleIndustryChange = (
    ...
    const updated = [...form.industries];
    updated[idx] = { ...updated[idx], [field]: value };
    setForm({ ...form, industries: updated });
  };

  const addIndustry = () => {
    setForm({ ...form, industries: [...form.industries, { ...defaultIndustry }] });
  };

  const removeIndustry = (idx: number) => {
    if (form.industries.length <= 1) return;
    setForm({ ...form, industries: form.industries.filter((_, i) => i !== idx) });
  };
  ...
  return (
    <div className="p-6 lg:p-8 max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-[#1A2530]">Edit Industries Section</h1>
        <p className="text-sm text-gray-500 mt-1">
          Edit judul, subtitle, dan daftar industri yang dilayani.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 bg-slate-50/50">
          <h2 className="text-lg font-bold text-[#1A2530]">Pengaturan Industries</h2>
        </div>
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Industri yang Kami Layani"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className="w-full border border-slate-200 rounded-xl p-3 text-sm focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none"
              placeholder="Subtitle industri..."
            />
          </div>

          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-xs font-semibold text-gray-600">Industri</label>
              <button
                onClick={addIndustry}
                className="inline-flex items-center gap-1 text-xs font-semibold text-amber-600 hover:text-amber-700"
              >
                <Plus size={14} /> Tambah Industri
              </button>
            </div>
            {form.industries.map((ind, idx) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-500">Industri #{idx + 1}</span>
                ...
                <input
                  ...
                  value={ind.name}
                  onChange={(e) => handleIndustryChange(idx, "name", e.target.value)}
                ...
                <input
                  ...
                  value={ind.icon}
                  onChange={(e) => handleIndustryChange(idx, "icon", e.target.value)}
                  className="w-full border border-slate-200 rounded-xl p-2.5 text-sm focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none"
                />
                <textarea
                  rows={2}
                  ...
                  onChange={(e) => handleIndustryChange(idx, "description", e.target.value)}
          ...
          >
            {saving ? (
              ...
              <>
                <Save size={16} /> Simpan Industries Section
              </>
            )}
          </button>
        </div>
