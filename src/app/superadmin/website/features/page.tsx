...
const defaultFeature: Feature = { icon: "", title: "", description: "" };
...
export default function FeaturesCMS() {
    ...
    getWebsiteSettings()
      .then((settings) => {
        ...
        setForm({
          ...
          title: (s.title as string) || "Fitur Unggulan",
          subtitle: (s.subtitle as string) || "",
          features:
            (s.features as Feature[])?.length
              ? (s.features as Feature[])
  ...
  const addFeature = () => {
    setForm({ ...form, features: [...form.features, { ...defaultFeature }] });
  };
  ...
  return (
    <div className="p-6 lg:p-8 max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-[#1A2530]">Edit Features Section</h1>
        <p className="text-sm text-gray-500 mt-1">
          Edit judul, subtitle, dan daftar fitur unggulan.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 bg-slate-50/50">
          <h2 className="text-lg font-bold text-[#1A2530]">Pengaturan Features</h2>
        </div>
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className="w-full border border-slate-200 rounded-xl p-3 text-sm focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none"
              placeholder="Subtitle fitur..."
            />
          </div>

          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-xs font-semibold text-gray-600">Fitur</label>
              <button
                onClick={addFeature}
            ...
            {form.features.map((feat, idx) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-500">Fitur #{idx + 1}</span>
          ...
          >
            {saving ? (
              ...
              <>
                <Save size={16} /> Simpan Features Section
              </>
            )}
          </button>
        </div>
