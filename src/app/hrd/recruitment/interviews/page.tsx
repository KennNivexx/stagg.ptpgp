...
import {
  ...
  FileText, CalendarDays, List, AlertTriangle, CheckCircle2, ChevronLeft, ChevronRight, Star,
} from "lucide-react";
...
export default function JadwalInterview() {
  ...
  // ── Multi-panel interview scoring (HR / User-Dept Manager / Director) ──
  const [scoreTarget, setScoreTarget] = useState<Interview | null>(null);
  const [scoreRows, setScoreRows] = useState<{ id: string; panel_role: string; panelist_name: string; score: number; notes: string | null }[]>([]);
  const [scoreAvg, setScoreAvg] = useState<number | null>(null);
  const [scoreForm, setScoreForm] = useState({ panel_role: "HR" as "HR" | "User/Dept Manager" | "Director", score: "80", notes: "" });
  const [savingScore, setSavingScore] = useState(false);

  const openScoring = async (iv: Interview) => {
    setScoreTarget(iv);
    setScoreForm({ panel_role: "HR", score: "80", notes: "" });
    const res = await getInterviewScores(iv.id);
    setScoreRows(res.scores as typeof scoreRows);
    setScoreAvg(res.average);
  };

  const handleSubmitScore = async () => {
    if (!scoreTarget) return;
    setSavingScore(true);
    const res = await submitInterviewScore(scoreTarget.id, scoreForm.panel_role, parseInt(scoreForm.score, 10) || 0, scoreForm.notes);
    setSavingScore(false);
    if ("error" in res) { showToast("err", res.error); return; }
    showToast("ok", "Penilaian interview tersimpan.");
    const refreshed = await getInterviewScores(scoreTarget.id);
    setScoreRows(refreshed.scores as typeof scoreRows);
    setScoreAvg(refreshed.average);
  };
  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      {/* Multi-panel interview scoring — HR, User/Dept Manager, dan Director
          masing-masing menilai independen (Enterprise Recruitment Management). */}
      {scoreTarget && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="font-extrabold text-slate-800 text-sm">Penilaian Interview</h3>
                <p className="text-[11px] text-slate-400 mt-0.5">{scoreTarget.full_name}</p>
              </div>
              <button onClick={() => setScoreTarget(null)} className="p-2 hover:bg-slate-100 rounded-lg text-slate-400"><X size={16} /></button>
            </div>

            {scoreRows.length > 0 && (
              <div className="mb-4 space-y-1.5">
                {scoreRows.map(r => (
                  <div key={r.id} className="flex items-center justify-between px-3 py-2 bg-slate-50 rounded-lg text-[11px]">
                    <span className="font-bold text-slate-700">{r.panel_role} <span className="font-normal text-slate-400">({r.panelist_name})</span></span>
                    <span className="font-extrabold text-slate-800">{r.score}</span>
                  </div>
                ))}
                {scoreAvg != null && (
                  <div className="flex items-center justify-between px-3 py-2 bg-amber-50 rounded-lg text-[11px] font-bold text-amber-700">
                    <span className="flex items-center gap-1"><Star size={11} /> Rata-rata Panel</span>
                    <span>{scoreAvg}</span>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Nilai Sebagai</label>
                <select value={scoreForm.panel_role} onChange={e => setScoreForm({ ...scoreForm, panel_role: e.target.value as typeof scoreForm.panel_role })} className="w-full border border-slate-200 p-2.5 rounded-xl text-sm">
                  <option value="HR">HR</option>
                  <option value="User/Dept Manager">User / Dept Manager</option>
                  <option value="Director">Director</option>
                </select>
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Skor (0-100)</label>
                <input type="number" min={0} max={100} value={scoreForm.score} onChange={e => setScoreForm({ ...scoreForm, score: e.target.value })} className="w-full border border-slate-200 p-2.5 rounded-xl text-sm" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Catatan</label>
                <textarea value={scoreForm.notes} onChange={e => setScoreForm({ ...scoreForm, notes: e.target.value })} className="w-full border border-slate-200 p-2.5 rounded-xl text-sm" rows={2} placeholder="Catatan penilaian..." />
              </div>
              <button onClick={handleSubmitScore} disabled={savingScore} className="w-full bg-[#CC0000] text-white py-2.5 rounded-xl text-sm font-bold hover:bg-[#aa0000] transition-colors disabled:opacity-50 inline-flex items-center justify-center gap-2">
                <Save size={14} /> {savingScore ? "Menyimpan..." : "Simpan Penilaian"}
              </button>
            </div>
          </div>
        </div>
                  ...
                  <div key={iv.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 hover:shadow-md transition-all">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <button onClick={() => openScoring(iv)} className="px-3 py-1.5 text-[10px] font-bold text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-lg transition-colors inline-flex items-center gap-1"><Star size={10} /> Nilai</button>
              ...
              <div className="space-y-2">
                {pastInterviews.slice(0, 5).map((iv) => (
                  <div key={iv.id} className="bg-white rounded-xl border border-slate-100 p-3 flex items-center gap-3 hover:bg-slate-50/30 transition-colors">
                    ...
                    <button onClick={() => openScoring(iv)} className="px-2.5 py-1 text-[9px] font-bold text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-lg transition-colors inline-flex items-center gap-1 shrink-0"><Star size={9} /> Nilai</button>
                    <span className="px-2 py-0.5 bg-slate-100 text-slate-500 rounded-full text-[9px] font-bold shrink-0">Selesai</span>
                  </div>
                ))}
              </div>
            </div>
