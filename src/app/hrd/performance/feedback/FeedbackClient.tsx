...
export default function FeedbackClient({
  ...
  const handleSubmit = async () => {
    ...
    const newEntry: FeedbackEntry = {
      ...
      employee_id: empId,
      reviewer_name: reviewer?.full_name || "Saya",
      category, rating, comment,
      created_at: new Date().toISOString(),
      employees: emp ? { full_name: emp.full_name, department: emp.department } : undefined,
    };
    setHistory(prev => [newEntry, ...prev]);
    setEmpId(""); setReviewerId(""); setCategory(""); setRating(0); setComment("");
  };
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <h4 className="text-sm font-bold text-slate-800">{emp?.full_name || "Karyawan"}</h4>
