...
import { Wrench, CheckCircle2, Clock, XCircle } from "lucide-react";
import { getKoreksiAbsensi, reviewKoreksi } from "@/app/actions/workforce-time";
...
export default function DeptCorrectionsPage() {
  const [data, setData] = useState<Koreksi[]>([]);
  const [loading, setLoading] = useState(true);
  const [toast, setToast] = useState("");
  const [statusFilter, setStatusFilter] = useState("");
  const [busyId, setBusyId] = useState<string | null>(null);

  useEffect(() => { getKoreksiAbsensi().then((d) => { setData(d as Koreksi[]); setLoading(false); }); }, []);

  const showToast = (msg: string) => { setToast(msg); setTimeout(() => setToast(""), 3000); };

  const doReview = async (id: string, approve: boolean) => {
    setBusyId(id);
    const result = await reviewKoreksi(id, approve);
    setBusyId(null);
    ...
    showToast(approve ? "Koreksi disetujui." : "Koreksi ditolak.");
  };
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-[#1A2530] mb-2">Koreksi Absensi Departemen</h1>
        <p className="text-sm text-gray-500">Setujui atau tolak pengajuan koreksi absensi karyawan di departemen Anda.</p>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Total Pengajuan", value: data.length, icon: <Wrench size={18} />, color: "bg-blue-50 text-blue-600" },
      ...
      {loading ? (
        ...
        <EmptyState icon={Wrench} title="Belum ada pengajuan koreksi." className="bg-white border-slate-100" />
      ) : (
              ...
              <tbody className="divide-y divide-slate-50">
                {filtered.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50/30">
                    ...
                    <td className="py-2.5 px-4 text-xs text-slate-600">{r.jenis_koreksi}</td>
