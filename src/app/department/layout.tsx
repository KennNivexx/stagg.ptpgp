...
export default function DepartmentLayout({ children }: { children: ReactNode }) {
  ...
  const [clientUserName, setClientUserName] = useState("Manajer Departemen");
  const [clientUserEmail, setClientUserEmail] = useState("dept@pratamagaluh.co.id");
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans text-slate-800">
      ...
      <aside className={`w-64 bg-emerald-900 text-white flex flex-col fixed inset-y-0 left-0 z-40 transition-transform duration-300 border-r border-emerald-950 lg:translate-x-0 lg:static lg:h-screen ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="p-5 border-b border-emerald-950 shrink-0">
          <div className="flex items-center gap-2">
            <span className="h-2.5 w-2.5 rounded-full bg-emerald-400 animate-pulse"></span>
            <span className="font-extrabold text-base tracking-wider bg-clip-text text-transparent bg-gradient-to-r from-white to-emerald-300">PT PGP &rarr; Dept Manager</span>
          </div>
          <p className="text-[10px] text-emerald-300 font-medium tracking-widest uppercase mt-0.5">Portal Departemen</p>
        </div>
          ...
          {MENU_GROUPS.map((group) => {
            ...
            return (
              <div key={group.label} className="mb-3">
                <div className={`flex items-center gap-2 px-5 py-1.5 ${hasActive ? "text-emerald-300" : "text-emerald-700"}`}>
                  <GroupIcon size={13} />
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      {isActive && <ChevronRight size={10} className="text-emerald-400 shrink-0" />}
        ...
        <div className="p-4 border-t border-emerald-950 bg-[#022C22] shrink-0">
          <div className="flex items-center gap-3 p-2 mb-3 rounded-lg bg-emerald-900/40">
            <div className="h-9 w-9 rounded-full bg-gradient-to-br from-emerald-500 to-teal-400 flex items-center justify-center text-white font-bold text-sm shadow-inner">
