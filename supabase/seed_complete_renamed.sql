-- ============================================================
-- PT PGP HRIS - Complete Database Setup (versi nama tabel Indonesia)
-- Copy-paste ke Supabase SQL Editor, lalu klik RUN.
--
-- Ini versi terbaru dari seed_complete.sql (nama tabel lama), disesuaikan
-- dengan migration 20260710008 & 20260710009 yang me-rename seluruh tabel
-- ke istilah Indonesia. Jalankan file ini HANYA setelah kedua migration
-- rename tersebut sudah dieksekusi — kalau dijalankan di database yang
-- masih pakai nama tabel lama, CREATE TABLE IF NOT EXISTS di sini akan
-- membuat tabel baru yang KOSONG dan terpisah dari tabel lama Anda.
-- ============================================================
...
-- 4. TABEL ORG UNITS ----------------------------------------------
CREATE TABLE IF NOT EXISTS unit_organisasi (
  id TEXT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS permintaan_sdm (
  id TEXT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS deskripsi_kerja (
  id TEXT PRIMARY KEY,
  position TEXT NOT NULL,
  department TEXT DEFAULT '',
...
CREATE TABLE IF NOT EXISTS spesifikasi_kerja (
  id TEXT PRIMARY KEY,
  position TEXT NOT NULL,
  department TEXT DEFAULT '',
...
-- Clean existing
DELETE FROM pengguna WHERE email != 'superadmin@ptpgp.co.id';
...
DELETE FROM unit_organisasi;
DELETE FROM departemen;

INSERT INTO unit_organisasi (id, code, name, parent_code, level, leader_name, leader_email, sort_order) VALUES
...
INSERT INTO jabatan (id, code, name, department, level) VALUES
...
DELETE FROM karyawan WHERE email != '__settings__@ptpgp.co.id';

-- System settings (required)
INSERT INTO karyawan (email, full_name, department, position, join_date, status, address) VALUES (
  '__settings__@ptpgp.co.id', 'System Settings', 'System', 'Settings', '2024-01-01', 'Tetap',
  '{"org_structure":[],"chart_layout":null}'
) ON CONFLICT (email) DO NOTHING;

-- HR & GA (7)
INSERT INTO karyawan (full_name, email, department, position, join_date, status, address) VALUES
...
SELECT
  ...
  (SELECT count(*) FROM karyawan WHERE email != '__settings__@ptpgp.co.id') as jumlah_karyawan,
  (SELECT count(*) FROM unit_organisasi) as jumlah_unit_organisasi,
  (SELECT count(*) FROM departemen) as jumlah_departemen,
  (SELECT count(*) FROM jabatan) as jumlah_jabatan;
